...
func enableProfileCmd(cmd *cmdutils.Cmd) {
	...
	cmd.FlagSetGroup.InFlagSet("General", func(fs *pflag.FlagSet) {
		fs.StringVarP(&opts.profileName, "name", "", "", "name or URL of the Quick Start profile. For example, app-dev.")
...
func doEnableProfile(cmd *cmdutils.Cmd, opts options) error {
	if cmd.NameArg != "" && opts.profileName != "" {
		return cmdutils.ErrNameFlagAndArg(cmd.NameArg, opts.profileName)
	}
	if cmd.NameArg != "" {
		opts.profileName = cmd.NameArg
	}
	if err := opts.validate(); err != nil {
		return err
	}

	profileRepoURL, err := repoURLForQuickstart(opts.profileName)
	if err != nil {
	...
	gitOps := gitops.Applier{
		...
		QuickstartName:   opts.profileName,
	}
